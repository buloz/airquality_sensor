...
{
    printf("Test BME680 :\n");
    printf("iaq: %f, temperature: %f, humidity: %f, pressure: %f", iaq, temperature, humidity, pressure);
}
...
int app_main()
{
    ...
    ret = bsec_iot_init(BSEC_SAMPLE_RATE_LP, 0.0f, bus_write, bus_read, sleep_loop, state_load, config_load);
    if (ret.bme680_status)
    {
        /* Could not intialize BME680 */
        printf("Couldn't initialize BME680\n");
        return (int)ret.bme680_status;
    }
    else if (ret.bsec_status)
    {
        /* Could not intialize BSEC library */
        printf("Couldn't initialize BSEC lib\n");
        return (int)ret.bsec_status;
    }
    
    /* Call to endless loop function which reads and processes data based on sensor settings */
    /* State is saved every 10.000 samples, which means every 10.000 * 3 secs = 500 minutes  */
    bsec_iot_loop(sleep_loop, get_timestamp_us, output_ready, state_save, 10000);
